...
page = open('dash_deptos\cuadros.html', 'r')
...
def input(tag, attrs):
	...
	if tag in useless_headers or tag in selfclosing:
		...
		start = '(id=\''+ att.get('id','') +'\',style={}, children=['
		...
		return parser.result.write('app.layout = ' + ident + 'html.' + 'div'.title() + start + '\n')
			...
			ident = ident_text * ident_lvl
			start = '(id=\''+ att.get('id','') +'\', href=\''+ att.get('href','') +'\', style={}, children=['
			ident_lvl = ident_lvl + 1
			classname[ident_lvl]=(', className=\''+ att.get('class','')+'\'')
